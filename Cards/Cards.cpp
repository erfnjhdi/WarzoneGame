...
void Card::play() {
  ...
  else if (cardType != CardType::C_Reinforcement) {
  ...
  if(card == nullptr){
    throw std::runtime_error("Hand did not contain card type: " + Card::getCardTypeString());
  }
...
for (int i = 0; i < 8; i++) {
  for (int j = 0; j < 5; j++) {
    // Type 1 = Bomb
    if (j == 0) {
      deckCards.push_back(new Card(CardType::C_Bomb, game));
    }
    // Type 2 = Reinforcement
    else if (j == 1) {
      deckCards.push_back(new Card(CardType::C_Reinforcement, game));
    }
    // Type 3 = Blockade
    else if (j == 2) {
      deckCards.push_back(new Card(CardType::C_Blockade, game));
    }
    // Type 4 = Airlift
    else if (j == 3) {
      deckCards.push_back(new Card(CardType::C_Airlift, game));
    }
    // Type 5 = Diplomacy
    else if (j == 4) {
      deckCards.push_back(new Card(CardType::C_Diplomacy, game));
    }
  }
}
}