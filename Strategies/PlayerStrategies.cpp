...
// create player based on type given
...
Order* Human::decideCard(Card* card) {
  switch(card->getCardType()) {
    case CardType::C_Reinforcement:
      return playReinforcementCard();
    case CardType::C_Bomb:
      return playBombCard();
    case CardType::C_Blockade:
      return playBlockadeCard();
    case CardType::C_Diplomacy:
      return playDiplomacyCard();
    case C_Airlift:
      return playAirliftCard();
  }
...
void Aggressive::playCard() {
  ...
  for(auto& card: *player->getHand()->getHandCards()){
      if(card->getCardType() == CardType::C_Bomb){
        card->play();
        return;
      }
  }
